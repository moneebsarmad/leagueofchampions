...
export async function calculatePointEconomy(
  ...
  const uniqueStudents = new Set(entries.map((e) => e.student_name).filter(Boolean))
...
export async function calculateConsistencyScore(
  ...
  const activeDays = schoolDays.filter((day) => dailyCounts.has(day)).length
  const activityRate = (activeDays / schoolDays.length) * 100

  // Calculate variance in daily counts
  const counts = schoolDays.map((day) => dailyCounts.get(day) || 0)
