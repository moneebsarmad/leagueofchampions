...
import { canonicalHouseName, schoolConfig } from '@/lib/school.config'

// Get houses from config
const HOUSES = schoolConfig.houses
...
export function analyzeStaffHouseBias(
  ...
  HOUSES.forEach((house) => {
    const schoolPct = totalSchoolPoints > 0 ? schoolHouseDistribution[house.name] / totalSchoolPoints : 0.25
    expectedDistribution[house.name] = totalStaffPoints * schoolPct
  })

  // Calculate chi-square
  const observed = HOUSES.map((h) => staffHousePoints[h.name] || 0)
  const expected = HOUSES.map((h) => expectedDistribution[h.name] || totalStaffPoints / HOUSES.length)
  const chiSquare = calculateChiSquare(observed, expected)

  const degreesOfFreedom = HOUSES.length - 1
  ...
  let maxDiff = 0
  HOUSES.forEach((house) => {
    const diff = staffHousePoints[house.name] - (expectedDistribution[house.name] || 0)
    if (diff > maxDiff) {
      maxDiff = diff
      favoredHouse = house.name
    }
  })
...
export async function generateStaffAnalytics(
  ...
  const [staffRes, entriesRes] = await Promise.all([
    supabase.from(Tables.staff).select('staff_name, email, house'),
    supabase
      .from(Tables.meritLog)
      .select('staff_name, house, r, points, date_of_event')
  ...
  HOUSES.forEach((house) => {
    schoolHouseDistribution[house.name] = 0
  })
  ...
  entriesByStaff.forEach((staffEntries, staffKey) => {
    const biasAnalysis = analyzeStaffHouseBias(staffEntries, schoolHouseDistribution)
...
export async function saveStaffAnalytics(records: StaffAnalyticsRecord[]): Promise<void> {
  ...
  const { error } = await supabase.from(Tables.staffAnalytics).upsert(
    ...
    { onConflict: 'staff_email,analysis_date,analysis_period' }
  )
